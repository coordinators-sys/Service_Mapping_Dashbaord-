...
# The form's own site choice list: submitted CODE -> the LABEL the enumerator
# selected. Generated by scripts/build_form_sites.py.
_FORM_SITES_PATH = os.path.join(_PROJECT_ROOT, "data", "form-sites.json")
...
def form_site_label(code):
    """The site NAME behind a form site code, or None if the code is not in
    the choice list. Never falls back to the code itself: a code masquerading
    as a name would be fed to the name-matching tiers as if it were one."""
    if not code:
        return None
    entry = _load_form_sites().get(str(code).strip())
    return entry.get("label") if entry else None


@lru_cache(maxsize=1)
def _load_form_sites() -> dict:
    if not os.path.isfile(_FORM_SITES_PATH):
        return {}
    with open(_FORM_SITES_PATH, encoding="utf-8") as f:
...
def parse_submission(raw: dict) -> ParsedSubmission:
    ...
    if site_name_select and str(site_name_select).strip().lower() != OTHER_SENTINEL:
        ...
        # The form stores a CODE; the enumerator saw and picked a NAME. Recover
        # that label so the site-name matching tiers have something to work
        # with and the dashboard can show a human-readable site instead of an
        # opaque code. This is reading the source correctly, not guessing — it
        # asserts no master-list identity. Resolution against the master list
        # stays a separate, evidence-based step, and the existing tiers still
        # flag an ambiguous name as needing review rather than matching it.
        site_name_raw = form_site_label(site_id_raw)
    else:
        site_name_raw = (
